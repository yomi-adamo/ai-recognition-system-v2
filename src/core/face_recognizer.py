...
class FaceRecognizer:
    ...
    def __init__(self, database_path: Optional[Union[str, Path]] = None):
        ...
        logger.info(f"FaceRecognizer initialized: {len(self.known_faces)} known faces, "
                   f"threshold={self.similarity_threshold}")
    
    @timing_decorator
    def add_known_face(self, name: str, image_path_or_encoding: Union[str, Path, np.ndarray]) -> bool:
        """
        ...
        try:
            ...
            if isinstance(image_path_or_encoding, (str, Path)):
                ...
                encodings = face_recognition.face_encodings(
                    image, num_jitters=self.encoding_jitter
                )
            ...
            logger.info(f"Added face for '{name}': {self.metadata[name]['face_count']} total encodings")
            return True
    ...
    def add_multiple_faces(self, faces_dict: Dict[str, List[Union[str, Path]]]) -> Dict[str, bool]:
        ...
        logger.info(f"Batch add complete: {sum(results.values())}/{len(results)} identities successful")
        return results
    ...
    def recognize_face(self, face_encoding: np.ndarray, 
        ...
        try:
            ...
            if len(matches) == 0:
                self._stats['unknown_faces'] += 1
                return {"name": "unknown", "confidence": 0.0, "reason": "no_matches_above_threshold"}
            ...
            logger.debug(f"Recognized face: {best_match['name']} "
    ...
    def recognize_faces_in_image(self, image_path: Union[str, Path], 
                                face_locations: Optional[List[Tuple]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def save_database(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Saved face database: {len(self.known_faces)} identities to {self.database_path}")
            return True
    ...
    def load_database(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded face database: {len(self.known_faces)} identities from {self.database_path}")
            return True
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'avg_encodings_per_identity': np.mean([len(encodings) for encodings in self.known_faces.values()]) if self.known_faces else 0,
            'recognition_accuracy': self._stats['successful_matches'] / max(1, self._stats['total_recognitions']),
